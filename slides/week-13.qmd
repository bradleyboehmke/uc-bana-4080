...
subtitle: "Week 13: Unsupervised Learning"
format:
  revealjs:
    self-contained: true
    slide-number: true
...
::: {.notes}
**Main Takeaway**: Today marks a major shift from supervised to unsupervised learning. We're moving from prediction tasks to discovery and pattern recognition.

**Key Points to Emphasize**:

- This is the last major topic before the final project
- Unsupervised learning is used extensively in industry for customer segmentation, feature engineering, and data exploration
- Both methods we'll cover today (clustering and PCA) are practical tools they'll likely use in their careers
- The concepts build on everything they've learned (standardization, train/test splits, model evaluation)
:::
...
::: {.notes}
**Main Takeaway**: Check understanding from Week 12 and create a bridge to unsupervised learning.

**Activity Management**:

- Give students the full 3 minutes to discuss
- Common topics from Week 12 that might come up:
  - Feature importance interpretation in Random Forests
  - When to use bagging vs. boosting
  - Hyperparameter tuning with GridSearchCV vs. RandomizedSearchCV
  - Cross-validation strategies

**Transition to Today**: After hearing from 2-3 groups, mention: "The ensemble methods we learned last week work with labeled data. Today we'll explore what to do when you DON'T have labels—that's unsupervised learning."
:::
...
::: {.notes}
**Main Takeaway**: Unsupervised learning is fundamentally different from everything we've done so far—we discover patterns instead of predicting outcomes.

**Key Teaching Points**:

- **The Big Shift**: Up until now, every model had a target variable (y). Now there's NO target—just features (X)
- **Success metrics change**: We can't measure accuracy or RMSE anymore. Success = interpretability + business value
- **Real-world context**: Often you collect data but don't know what patterns exist yet. Unsupervised learning helps you explore and understand your data
- **Examples to mention**:
  - Supervised: "Predict which customers will churn" (you need historical churn labels)
  - Unsupervised: "Find natural customer segments" (no predetermined groups needed)
- **Common student question**: "How do we know if we're right?" Answer: "We validate with domain experts and measure business impact, not statistical accuracy"
:::
...
::: {.notes}
**Main Takeaway**: Unsupervised learning is valuable when you have data but no predetermined labels or categories.

**Example Answers to Listen For**:

1. **Retailer example (given)**: Millions of transactions, want to discover customer segments for targeted marketing
2. **Healthcare**: Patient records with symptoms/treatments, discover patient subtypes that respond differently to treatments
3. **E-commerce**: Product catalog, group similar products for recommendations (no one manually labeled "similar products")
4. **Social media**: User behavior data, find communities or interest groups
5. **Manufacturing**: Sensor data from machines, detect anomaly patterns (don't know failure modes ahead of time)
6. **Document organization**: Thousands of documents, automatically organize into topics

**What Makes These Unsupervised?**:

- No pre-existing labels for "customer type" or "patient subtype"
- You're discovering the structure, not predicting a known outcome
- Value comes from finding patterns you didn't know existed

**Transition**: "Great examples! Notice how in each case, we have rich data but no labels. That's when unsupervised learning shines."
:::
...
::: {.notes}
**Main Takeaway**: There are two fundamentally different types of unsupervised learning—clustering works on ROWS, dimension reduction works on COLUMNS.

**Critical Distinction** (emphasize heavily):

- **Clustering**: "Which customers are similar to each other?" → Groups rows
- **Dimension Reduction**: "Which features capture similar information?" → Combines columns
- This is a common point of confusion—make sure students understand the difference!

**Today's Focus**:

- K-Means clustering: Most widely used clustering algorithm in business
- PCA: Most common dimension reduction technique
- These are the "workhorses"—students will see them constantly in industry

**Diagram Interpretation**:

- Point out the other methods listed (Hierarchical, DBSCAN, t-SNE, Autoencoders) to show there's a whole ecosystem
- But K-Means and PCA are the foundations that everyone should know
- The other methods build on or extend these core ideas
:::
...
::: {.notes}
**Main Takeaway**: Clustering operates on ROWS—it finds groups of similar observations.

**Visual Walkthrough**:

- Point to the matrix visualization showing 8 rows (customers) and 18 features
- Notice how entire ROWS are color-coded together (Premium = red, Regular = teal, Inactive = blue)
- Each row represents one customer across all their features
- The algorithm looked across all 18 features to determine "these 3 customers are similar"

**Emphasize**:

- The number of rows stays the same (8 customers → still 8 customers)
- We're just adding a new piece of information: cluster membership
- Each customer now has a label: Premium, Regular, or Inactive
- This labeling happened automatically based on patterns in the features

**Real-World Application**:
"If this were a real retailer, you might find that Premium customers all have high spending, frequent visits, and low discount usage. The algorithm discovered this pattern without being told what makes a customer 'premium.'"
:::
...
::: {.notes}
**Main Takeaway**: Dimension reduction operates on COLUMNS—it combines correlated features into composite components.

**Visual Walkthrough**:

- Original matrix: 8 rows × 5 features (Sq Ft, Bedrooms, Bathrooms, Location Score, School Rating)
- Notice the color coding: Red features (Sq Ft, Bedrooms, Bathrooms) → PC1 "Overall Size"
- Teal features (Location Score, School Rating) → PC2 "Location Quality"
- After PCA: 8 rows × 2 components (60% fewer features!)

**Key Contrast with Clustering**:

- **Clustering changed**: ROWS (which customers are grouped together)
- **PCA changes**: COLUMNS (which features are combined)
- **Clustering output**: Same features + cluster label
- **PCA output**: New features (principal components)

**House Example Explanation**:
"Sq Ft, bedrooms, and bathrooms are all highly correlated—big houses have more of everything. Instead of storing all three, we can create ONE composite measure called 'Overall Size' (PC1) that captures most of that information. Similarly, location score and school rating both relate to neighborhood quality, so we combine them into PC2."

**Emphasize**: We went from 5 features to 2 components while keeping the same 8 houses!
:::
...
::: {.notes}
Emphasize that clustering finds groups that exist naturally in the data—we don't tell it what the groups should look like ahead of time.
:::
...
ax.scatter(centroids_final[:, 0], centroids_final[:, 1],
          s=500, c=centroid_color, marker='*', edgecolors='black',
          linewidth=2, zorder=5)
ax.set_title('Step 6: Converged!\nCentroids stop moving → Done', fontsize=10, fontweight='bold')
ax.set_xlim(0, 7)
ax.set_ylim(0, 7)
ax.grid(True, alpha=0.3)
ax.set_aspect('equal')
...
::: {.notes}
**Main Takeaway**: Learn to distinguish clustering problems from supervised learning problems.

**Correct Answers**:

- **A (Customer purchase history → marketing campaigns)**: CLUSTERING
  - Why: No predetermined segments exist
  - Goal: Discover natural customer groups based on behavior
  - Then create targeted campaigns for each discovered segment

- **B (Predict customer churn)**: SUPERVISED LEARNING
  - Why: You need historical data with churn labels (churned = yes/no)
  - Goal: Predict a specific outcome (will this customer churn?)
  - This requires a target variable

- **C (Organize 10,000 products into groups)**: CLUSTERING
  - Why: No predetermined product categories given
  - Goal: Find natural groupings based on purchase patterns, attributes, etc.
  - Similar products should end up in same cluster

- **D (Predict house prices)**: SUPERVISED LEARNING
  - Why: You have historical house prices (target variable)
  - Goal: Predict a specific numeric outcome (price)
  - Classic regression problem

**Teaching Strategy**:

- Have students vote (raise hands for clustering vs. supervised for each)
- Ask "What's the key difference?" → Presence/absence of a target variable
- Emphasize: If you're PREDICTING something specific → supervised; if you're DISCOVERING patterns → unsupervised

**Common Student Confusion**:
"Can't we use clustering for A and then predict churn later?" → YES! That's actually a great workflow: cluster first to find segments, THEN build supervised models for each segment. But the clustering step itself is unsupervised.
:::
...
::: {.notes}
**Main Takeaway**: K-Means requires you to choose K upfront, but we have two complementary methods to help make this decision.

**Elbow Method - Deep Explanation**:

- **What is inertia?** Sum of squared distances from each point to its cluster center
- **Why does it decrease?** More clusters = points closer to their centers on average
- **The "elbow"**: The point where adding more clusters gives diminishing returns
- **How to spot it**: Look for the "bend" where the curve starts flattening out
- **Limitation**: Sometimes there's no clear elbow (we'll see this with real data!)

**Silhouette Score - Deep Explanation**:

- **What it measures**: For each point, how similar is it to its own cluster vs. the nearest other cluster?
- **Score = +1**: Perfect! Point is very close to its cluster center and far from others
- **Score = 0**: Point is on the boundary between two clusters (could go either way)
- **Score = -1**: Wrong cluster! Point is closer to a different cluster
- **Overall score**: Average across all points
- **Good scores**: 0.5-0.7 for well-separated clusters; 0.2-0.4 is acceptable for messy real-world data

**Key Teaching Point**:

"These are GUIDES, not rules. The best K depends on:

1. Statistical fit (elbow & silhouette)
2. Business needs (can marketing handle 5 segments or only 3?)
3. Interpretability (can you clearly describe each cluster?)
4. Actionability (can you do something different for each segment?)"

**Common Question**: "Which metric should I trust more?" → Use BOTH together. If they agree, great! If they disagree, try a few values of K and see which produces the most interpretable segments.
:::
...
::: {.notes}
**Main Takeaway**: Practice interpreting elbow plots and balancing statistical vs. business considerations.
...
- Look for the biggest drop in inertia:
  - K=2 to K=3: Drop of 3,000 (10,000 → 7,000) — **HUGE drop (30%)**
  - K=3 to K=4: Drop of 1,800 (7,000 → 5,200) — Still significant (26%)
  - K=4 to K=5: Drop of 400 (5,200 → 4,800) — **Flattening out (8%)**
  - K=5 to K=6: Drop of 200 (4,800 → 4,600) — Very small (4%)
- **The elbow is around K=3 or K=4** (where the curve starts to flatten)

**2. What K would you recommend? Why?**

- **Statistical answer**: K=3 or K=4
  - K=3 gives you the biggest "bang for your buck" (biggest drop before flattening)
  - K=4 still shows meaningful improvement
  - K=5+ shows diminishing returns
- **Practical answer**: "It depends on what the segments mean"
  - Need to actually look at the cluster profiles
  - Are they interpretable? Actionable?

**3. What if marketing says they can only handle 3 segments?**

- **Critical teaching moment**: Business constraints trump statistical optimization!
- Even if K=4 is statistically "better," K=3 is the right answer if:
  - Marketing team doesn't have resources for 4 campaigns
  - Leadership can't remember/act on 4 different strategies
  - The cost of managing 4 segments exceeds the benefit
- **Real-world insight**: "In industry, you'll often have to choose a 'good enough' solution that's actually implementable over a 'perfect' solution that won't be used."

**Broader Lesson**:
"Data science isn't just about finding the optimal statistical answer. It's about finding the BEST answer given real-world constraints: resources, time, budget, organizational complexity, and stakeholder understanding."
:::
...
::: {.notes}
**Main Takeaway**: Prepare students for the reality that real-world clustering often doesn't have clear, obvious solutions.

**Critical Mindset Shift**:

- **So far**: Toy examples with clean, well-separated clusters
  - Synthetic customer data with 3 obvious segments
  - Perfect elbow at K=3
  - High silhouette scores (0.5-0.7)
  - Clear interpretation

- **Reality**: Real business data is messy
  - Customers exist on a SPECTRUM, not in neat boxes
  - No clear elbow (gradual decline)
  - Low silhouette scores (0.15-0.20)
  - Overlapping patterns

**Why This Matters**:

- **Student expectation**: "I'll run K-Means and get obvious segments with high scores"
- **Reality**: "I'll get ambiguous results that require judgment and domain expertise"
- **Goal of this section**: Set realistic expectations so students don't get discouraged

**Upcoming Real-World Dataset**:

- **Complete Journey**: Actual grocery store transaction data
- **801 households**: Real people with messy, inconsistent behavior
- **14 features**: Mix of behavioral (spending, frequency) and demographic (age, income)
- **Result**: No clear "winner" for K, low silhouette scores across the board

**Teaching Strategy**:

1. Show the messy results AFTER showing clean examples
2. Emphasize: "This doesn't mean you did something wrong!"
3. Explain: "Real customer behavior is complex and doesn't fit into neat boxes"
4. Reassure: "Low silhouette scores (0.15-0.20) are NORMAL and ACCEPTABLE for behavioral data"

**Key Message to Students**:
"When you get 'messy' results in your own projects, don't panic! This is what real data science looks like. The skill is in interpreting ambiguous results, combining statistical metrics with business context, and making defensible decisions even when there's no clear 'right answer.'"
:::
...
::: {.notes}
**Main Takeaway**: Recognize the problems caused by having too many features and understand the trade-offs of dimension reduction.

**Question Analysis**:

**1. Which problems suggest you have too many features?**

- **Correlated features**: Yes! Multicollinearity causes unstable models
  - If sq ft and bedrooms are correlated, they provide redundant information
  - Models can't distinguish their individual effects
  - Feature weights become unreliable
- **Long training time (30 minutes)**: Yes! More features = more computation
  - 100 features vs. 10 features = 10× more parameters to estimate
  - Exponentially slower for some algorithms
- **Test accuracy worse than training**: Yes! Classic sign of overfitting
  - With 100 features and limited data, model memorizes noise
  - High-dimensional spaces are sparse (curse of dimensionality)
  - Model can't generalize to new data

**2. What might you gain by reducing from 100 to 10 features?**

- **Faster training**: 10-100× speedup depending on algorithm
- **Better generalization**: Less overfitting, better test performance
- **Simpler models**: Easier to understand, explain, and deploy
- **Visualization**: Can actually plot and explore the data
- **Less multicollinearity**: Components are uncorrelated by design
- **Noise reduction**: Minor components often represent measurement noise

**3. What might you lose?**

- **Some information**: PCA with 10 components might capture 90% of variance, but you lose 10%
- **Interpretability**: Original features ("sq ft") are intuitive; PC1 is a weighted combination that's harder to explain
- **Specific details**: Rare patterns captured by minor components are lost
- **Reversibility**: You can't perfectly reconstruct the original 100 features from 10 components

**Key Teaching Insight**:
"PCA is always a trade-off. You're betting that the information you LOSE (10-20% of variance, usually noise) is worth the benefits you GAIN (speed, simplicity, generalization). In practice, this trade-off is almost always favorable for high-dimensional data."
:::
...
::: {.notes}
**Main Takeaway**: PCA discovers underlying "themes" or "factors" that explain why multiple features are correlated.

**Analogy Walkthrough**:

**The Setup**:

- Imagine you're tracking 10 apartment features for every listing
- Many features are correlated (not independent):
  - Big apartments → more bedrooms, bathrooms, closets
  - New buildings → farther from city center, higher rent
  - High floors → better views, premium pricing

**What PCA Discovers**:

**PC1: "Size & Space" (50% of variance)**

- This component captures the SHARED PATTERN in: sq footage, bedrooms, bathrooms, closet space
- **Why are these correlated?** They all increase together as apartments get larger
- **What PC1 represents**: A single "size score" that summarizes all size-related features
- **Example**: Apartment A (PC1 = 2.5) vs. Apartment B (PC1 = -1.2)
  - A is large on ALL size dimensions
  - B is small on ALL size dimensions
  - One number tells the whole story!

**PC2: "Location & Age" (25% of variance)**

- Captures: distance to downtown, building age, neighborhood rating
- **Why correlated?** Older buildings tend to be downtown in better-established neighborhoods
- **What PC2 represents**: Urban vs. suburban character
- **High PC2**: Downtown, older, established neighborhood
- **Low PC2**: Suburban, newer, developing area

**PC3: "Luxury & Amenities" (15% of variance)**

- Captures: floor number, finishes, gym/pool access
- **Why correlated?** Luxury features tend to come together in premium buildings
- **What PC3 represents**: Luxury tier
- **High PC3**: High-floor unit with premium finishes and amenities
- **Low PC3**: Basic unit with standard features

**The Power of PCA**:

- **Original**: 10 features to track and compare
- **After PCA**: 3 components capture 90% of variation
- **Benefit**: Much easier to compare apartments, make decisions, and find similar listings
- **The key**: PCA found these patterns automatically—didn't need domain expertise to group "size features" together

**Teaching Point**:
"PCA isn't doing magic—it's just finding correlations and grouping correlated features together. Humans could have done this manually, but PCA does it mathematically and optimally."
:::
...
::: {.notes}
Emphasize that standardization is CRITICAL for PCA—otherwise features with large values will dominate. After PCA, we still have 30 components (same as features), but now they're uncorrelated and ordered by variance. Show how the data looks completely different in PC space.
:::
...
::: {.notes}
**Main Takeaway**: Practice reading scree plots and making informed decisions about how many components to retain.

**Question Analysis**:

**1. How many components would you keep? Why?**

Let's calculate cumulative variance for each option:

- **PC1 only**: 35% variance — Too little information!
- **PC1-2**: 35% + 20% = 55% variance — Still losing half the information
- **PC1-3**: 55% + 12% = 67% variance — Getting better, but below typical threshold
- **PC1-4**: 67% + 8% = **75% variance** — Good threshold, retained 3/4 of information
- **PC1-5**: 75% + 6% = **81% variance** — Even better, common threshold
- **PC1-6+**: 81% + 19% = 100% variance — Keeping 45 components for last 19%? Diminishing returns!

**Recommended answer**: **Keep PC1-5 (81% variance)**

- **Common rule of thumb**: Aim for 80-90% cumulative variance
- **PC1-4 (75%)**: Also defensible if simplicity is paramount
- **PC1-10**: Might be justified if you really need that extra precision

**2. If you keep just PC1 and PC2, what percentage do you retain?**

- **Answer**: 35% + 20% = **55% of variance**
- **Is this enough?**
  - For visualization: Yes! Often we reduce to 2-3 components just to plot
  - For modeling: Probably not—you're losing 45% of information
  - For exploration: Maybe—depends if those 2 components capture the main patterns

**3. What are you trading off when you reduce from 50 to 5 features?**

- **Gain**: 90% reduction in features (50 → 5)
- **Lose**: 19% of variance (81% retained, 19% lost)
- **The trade-off**:
  - That 19% you're losing is spread across 45 components (PC6-50)
  - Each of those captures < 0.5% of variance on average
  - Likely represents noise, measurement error, or very subtle patterns
  - The first 5 components capture the "signal," the rest is mostly "noise"

**Key Teaching Points**:

- "There's no universal 'right' answer—it depends on your goals"
- "For modeling: usually keep components up to 80-90% variance"
- "For visualization: often just keep 2-3 components"
- "Look for the 'elbow' in scree plot—where adding more components gives diminishing returns"
- "In this example, PC6+ each add < 1% variance—probably not worth the complexity"

**Real-World Context**:
"Imagine you're building a recommendation system. You might reduce 10,000 product features to 50 components (99% variance) for speed, or reduce to 10 components (95% variance) if you need real-time predictions. The choice depends on your business requirements!"
:::
...
**Interpretation**: PC1 loads heavily on features related to tumor size, texture, and shape irregularity. It represents **"overall tumor severity"**—high PC1 means larger, more irregular tumors.

::: {.notes}
**Main Takeaway**: Principal components are interpretable by examining which original features contribute most strongly (highest loadings).

**What are "loadings"?**

- **Loadings** = weights that show how much each original feature contributes to a principal component
- **High absolute loading** = that feature strongly influences this component
- **Positive loading** = feature increases with the component
- **Negative loading** = feature decreases with the component
- **Near-zero loading** = feature doesn't contribute much to this component

**How to Interpret PC1 in Breast Cancer Data**:

Looking at the top features from the code output, we'll see:

1. **radius_worst** (high loading): Largest radius measurement
2. **perimeter_worst** (high loading): Largest perimeter
3. **area_worst** (high loading): Largest area
4. **concave points_worst** (high loading): Most severe concavity
5. **radius_mean** (high loading): Average radius
6. **perimeter_mean** (high loading): Average perimeter

**What does this tell us?**

- PC1 is dominated by SIZE and IRREGULARITY measurements
- All the "worst" measurements (most extreme values) load heavily
- **Interpretation**: PC1 represents "Overall Tumor Severity"
  - High PC1 score → Large tumor with irregular features → Likely malignant
  - Low PC1 score → Small tumor with regular features → Likely benign

**Teaching Strategy**:

1. Show the loadings table from the code output
2. Ask: "What do these top features have in common?" → All relate to size/shape
3. Explain: "PC1 is a weighted average of these features—a single score summarizing tumor severity"
4. Contrast with original features: "Instead of looking at 30 separate measurements, PC1 gives you ONE number that captures the most important pattern"

**Why This Matters**:

- PCA creates new features, but they're NOT random or meaningless
- Each component has an interpretation based on its loadings
- Good practice: Always examine loadings to understand what your components represent
- You can name components based on their interpretation (e.g., "Tumor Severity Score")

**Common Student Question**:
"Are all loadings equally important?" → No! Focus on the highest absolute loadings—those features dominate that component. Small loadings (< 0.1) contribute very little.
:::
...
plt.ylabel(f'PC2 ({pca_2d.explained_variance_ratio_[1]*100:.1f}% variance)',
           fontweight='bold', fontsize=11)
plt.title('Breast Cancer Data: 30 Features → 2 Components',
...
- **High PC1 (moving right)** = Larger tumors with more irregular shapes → More likely MALIGNANT (red points)
- **Low PC1 (moving left)** = Smaller, more regular tumors → More likely BENIGN (teal points)
...
- Ask: "If you had to diagnose with just these 2 numbers (PC1, PC2), how would you do it?" (Answer: High PC1 → likely malignant)
:::
...
::: {.notes}
This is a very common pattern in practice. Clustering and dimension reduction as preprocessing steps for prediction models. Emphasize that PCA and clustering aren't just standalone techniques—they're often combined and used to engineer features for supervised learning.
:::
...
::: {.notes}
**Main Takeaway**: Learn to choose the right unsupervised learning method based on your goal.

**Decision Framework**:

**Start Here**: "I have unlabeled data" → Now what?

**Goal 1: Find groups in my data (Use Clustering)**

- **When to use**:
  - "I want to discover customer segments"
  - "I need to organize products into categories"
  - "I want to identify market segments"
  - "I'm looking for patterns in customer behavior"
- **Key question**: "Which observations are similar to each other?"
- **Output**: Cluster labels for each observation
- **Example use cases**:

  - Customer segmentation for targeted marketing
  - Product recommendation (group similar products)
  - Anomaly detection (points that don't fit any cluster)
  - Document organization (group similar articles)

**Goal 2: Reduce number of features (Use Dimension Reduction / PCA)**

- **When to use**:
  - "I have 100+ features and want to simplify"
  - "My features are highly correlated"
  - "Training takes too long"
  - "I want to visualize high-dimensional data"
  - "I'm worried about overfitting"
- **Key question**: "Which features capture similar information?"
- **Output**: New set of uncorrelated principal components
- **Example use cases**:

  - Feature engineering before supervised learning
  - Data visualization (reduce to 2-3 components for plotting)
  - Noise reduction (keep components, discard noise)
  - Compression for faster computation

**Goal 3: Both! (Use PCA then Clustering)**

- **When to use**:
  - "I have many correlated features AND want to find segments"
  - "Clustering on 100 features is computationally expensive"
  - "I want to remove noise before clustering"
- **Workflow**:

  1. Apply PCA to reduce 100 features → 20 components (faster, less noisy)
  2. Apply K-Means to those 20 components (find customer segments)
  3. Use segments as features for supervised learning (predict churn for each segment)
- **Example use cases**:

  - Customer segmentation with behavioral + demographic data
  - Image clustering (reduce pixel features with PCA, then cluster)
  - Gene expression analysis (reduce 20,000 genes → 50 components → cluster patients)

**Decision-Making Questions**:

1. "What's your PRIMARY goal?"
   - Discover groups → Clustering
   - Simplify features → PCA
   - Both → PCA first, then Clustering

2. "Do you have too many features?"
   - Yes → Consider PCA first
   - No → Go straight to clustering if you want groups

3. "Are your features highly correlated?"
   - Yes → PCA will help a lot
   - No → Maybe feature selection instead of PCA

**Real-World Insight**:
"In practice, the combo approach (PCA → Clustering → Supervised Learning) is very common. It's like a data science pipeline: reduce complexity, discover patterns, then predict outcomes."
:::

...
In this chapter, we focus on **exploratory visualization using Pandas**, a quick and powerful way to visually explore data and uncover insights — all within the same environment where your analysis lives. In future chapters, we'll dive deeper into refining visualizations for **explanatory purposes**, showing how libraries like **Seaborn**, **Matplotlib**, and **Bokeh** can help you tell compelling, professional-grade data stories.

By the end of this chapter, you will be able to:

* Use the Pandas `.plot` API to quickly generate exploratory visualizations from Series and DataFrames
* Visualize distributions, group comparisons, and trends using common plot types (histograms, bar charts, line plots, scatter plots)
* Create time-series visualizations using `.resample()`
* Apply basic Matplotlib styling to polish Pandas plots
...
As you read through this chapter, we encourage you to follow along using the [companion notebook](https://github.com/bradleyboehmke/uc-bana-4080/blob/main/notebooks/examples/13_data_viz_pandas.ipynb) in Google Colab (or other editor of choice). This interactive notebook lets you run code examples covered in the chapter—and experiment with your own ideas.

👉 Open the [Pandas Data Viz Notebook in Colab](https://colab.research.google.com/github/bradleyboehmke/uc-bana-4080/blob/main/notebooks/examples/13_data_viz_pandas.ipynb).
:::
...
In the **next chapter**, we’ll survey the broader Python visualization ecosystem — introducing three libraries that each unlock a different kind of capability. **Seaborn** makes statistical relationships easy to see with minimal code. **Matplotlib** gives you full control over every element of a figure, making it the go-to for publication-quality and presentation-ready charts. **Bokeh** produces interactive, web-ready visualizations that let stakeholders explore data themselves. You’ll see each library in action and learn when to reach for each one.

Then, in the final chapter of this module, we’ll put everything together. Using the Complete Journey dataset, we’ll walk through a realistic **exploratory data analysis** from start to finish — combining the data wrangling skills from earlier modules with the visualization tools from this one to uncover insights and tell a coherent data story. That case study will also serve as a model for your own midterm project analysis.

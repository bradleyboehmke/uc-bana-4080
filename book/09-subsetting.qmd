...
When working with real data, you rarely need everything in a dataset. You might want only certain columns, or only the rows that meet a specific condition — maybe all transactions from a particular customer segment, or flights operated by a single airline. This process of extracting the relevant portion of a DataFrame is called **subsetting**, and it's one of the most fundamental skills in data analysis.
...
As you read through this chapter, we encourage you to follow along using the [companion notebook](https://github.com/bradleyboehmke/uc-bana-4080/blob/main/notebooks/examples/09_subsetting.ipynb) in Google Colab (or other editor of choice). This interactive notebook lets you run code examples covered in the chapter—and experiment with your own ideas.

👉 Open the [Subsetting Notebook in Colab](https://colab.research.google.com/github/bradleyboehmke/uc-bana-4080/blob/main/notebooks/examples/09_subsetting.ipynb).
:::
...
Now that you can import, inspect, and subset a DataFrame, it's time to apply those skills to the **Complete Journey** dataset — the retail grocery data you'll use throughout the midterm project.
...
Use the techniques from Chapters 7–9 to explore this data:

1. **How big is each dataset?** Report the number of rows and columns for `transactions`, `products`, and `demographics` using `.shape`.
2. **What variables are available?** Use `.columns` and `.dtypes` to inventory the key fields in `transactions`.
3. **First look at purchases:** Filter `transactions` to rows where `sales_value` is greater than 10. How many transactions meet this threshold?
4. **Focus a question:** Using `transactions`, find all purchases in a single week (use the `week` column). How many transactions occurred in week 1?
